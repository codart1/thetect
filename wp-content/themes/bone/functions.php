	...
	function md_bone_scripts(){
	    ...
	    switch ($md_bone_deploy_mode) {
	        default: //deploy
	        	...
	        	wp_enqueue_style('cm', get_template_directory_uri(). '/css/cm.css');

	        	$layout_opt = md_bone_get_layout_opt();
	        	if ($layout_opt['content-layout'] === 'masonry') {
					wp_enqueue_script( 'masonry', '', array(), '', true );
				}
	    ...
	    wp_enqueue_script('cm-script', get_template_directory_uri() . '/js/cm/cm.js', array('jquery'), MINIMALDOG_THEME_VERSION, true);
	}
}
...
if ( !function_exists('md_bone_frontend_variable') ){
	function md_bone_frontend_variable(){
		...
		if ($query_vars == '[]') {
			$query_vars = '{"post_status":"publish","posts_per_page":"'.$posts_per_page.'"}';
		}
...
//CM
get_template_part( 'inc/cm/helper' );
...
function md_bone_widgets_init(){
	...
	register_sidebar(array(
		'id' => 'adsidebar-1',
		'name' => esc_html__('Ad Sidebar 1', 'bone'),
	...
	register_sidebar(array(
		'id' => 'adsidebar-2',
		'name' => esc_html__('Ad Sidebar 2', 'bone'),
	...
	register_sidebar(array(
		'id' => 'adsidebar-3',
		'name' => esc_html__('Ad Sidebar 3', 'bone'),
...
if ( !function_exists('md_bone_custom_css') ){
    function md_bone_custom_css() {
    	if (!is_admin()) {
    		...
    		$custom_css .= $border_color_selectors.'{border-color:'.md_bone_get_option('primary-color', '#81c483').';}';
    		$custom_css .= '.reviewMeter-item-score:after,.commentCountBox:before{border-top-color:'.md_bone_get_option('primary-color', '#81c483').';}';
	...
	function md_bone_featured_img( $size = 'md_bone_md', $class = '', $icon = false ) {
		...
		<div class="postFeaturedImg<?php if ($class != '') echo esc_attr($class); ?>"<?php if ( is_single() ) { echo ' itemprop="image" itemscope itemtype="https://schema.org/ImageObject"'; } ?>>
			...
			<?php } else {
				...
				$post_image = wp_get_attachment_image_src( get_post_thumbnail_id(), 'md_bone_sm' );
		...
		switch ($style) {
			...
			case '3':
				...
				<div class="postMeta--btn postMeta--btn--3 clearfix">
					<div class="u-floatLeft">
						<a href="<?php echo esc_url($article_link); ?>" class="readMoreLink primaryColor metaFont"><?php esc_html_e('Read more', 'bone'); ?></a>
					</div>
					<div class="u-floatRight">
						<a href="<?php comments_link(); ?>" class="metaFont metaLink">
							<i class="fa fa-comment-o"></i>
...
if ( !function_exists('md_bone_post_meta_author') ){
	function md_bone_post_meta_author( $type = '2', $size = 36 ) {
		...
		<div class="postMeta--author postMeta--author--1 o-media">
			<div class="postMeta--author-avatar o-media-left o-media--middle"><?php echo get_avatar( get_the_author_meta( 'ID' ), $size, '', esc_html__( 'avatar', 'bone' ) ); ?></div>
			<div class="postMeta--author-text o-media-body o-media--middle">
				<span class="postMeta--author-author metaFont"><?php the_author_posts_link(); ?></span><span class="middot">&middot;</span><span class="metaText metaDate"><abbr <?php if (is_single()) { echo 'itemprop="datePublished" content="'.get_the_time('Y-m-d').'" '; } ?>class="published" title="<?php the_time(get_option( 'date_format' )); ?>"><?php md_bone_human_datetime(); ?></abbr></span>	
			</div>
		</div>
		<?php
		} elseif ( $type == '2') {
		?>
		<div class="postMeta--author postMeta--author--2 o-media">
			<div class="postMeta--author-avatar o-media-left o-media--middle"><?php echo get_avatar( get_the_author_meta( 'ID' ), $size, '', esc_html__( 'avatar', 'bone' ) ); ?></div>
			<div class="postMeta--author-text o-media-body o-media--middle">
				<div class="postMeta--author-author metaFont"><?php the_author_posts_link(); ?></div>
				<span class="metaText metaDate"><abbr <?php if (is_single()) { echo 'itemprop="datePublished" content="'.get_the_time('Y-m-d').'" '; } ?>class="published" title="<?php the_time(get_option( 'date_format' )); ?>"><?php md_bone_human_datetime(); ?></abbr></span>	
			</div>
		</div>
		<?php
		} elseif ( $type == '3') {
		?>
		<div class="postMeta--author postMeta--author--3 o-media">
			<div class="postMeta--author-avatar o-media-left o-media--middle"><?php echo get_avatar( get_the_author_meta( 'ID' ), $size, '', esc_html__( 'avatar', 'bone' ) ); ?></div>
			<div class="postMeta--author-text o-media-body o-media--middle">
				<div class="postMeta--author-author metaFont"><?php the_author_posts_link(); ?></div>
			</div>
				...
				<div class="postMeta postMeta--3 flexbox">
					...
					<div class="flexbox-item">
						<div>
							...
							<div class="metaFont metaDate"><abbr <?php if (is_single()) { echo 'itemprop="datePublished" content="'.get_the_time('Y-m-d').'" '; } ?>class="published" title="<?php the_time(get_option( 'date_format' )); ?>"><?php echo get_the_date(); ?></abbr></div>	
						</div>
					</div>
					<div class="flexbox-item">
						<?php
        	...
        	case 'wide': ?>
        		<header class="postSingle-header postSingle-header--wide postSingle-header--big">
        			<div class="container">
						...
						<h1 itemprop="<?php echo esc_html($title_schema); ?>" class="postTitle"><?php the_title(); ?></h1>
...
if ( !function_exists('md_bone_get_sharing') ){
	function md_bone_get_sharing($args) {	
		...
		$description = !empty($description) ? ' addthis:description="'.wp_trim_words($description, 24).'"' : '';
